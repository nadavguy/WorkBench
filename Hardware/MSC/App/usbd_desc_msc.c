...
#define USBD_LANGID_STRING_MSC   1033
#define USBD_MANUFACTURER_STRING_MSC     "STMicroelectronics"
#define USBD_PID_MSC_FS     22314
#define USBD_PRODUCT_STRING_MSC_FS     "STM32 Mass Storage"
#define USBD_CONFIGURATION_STRING_MSC_FS     "MSC Config"
#define USBD_INTERFACE_STRING_MSC_FS     "MSC Interface"
...
static void Get_SerialNum_MSC(void);
static void IntToUnicode_MSC(uint32_t value, uint8_t * pbuf, uint8_t len);
...
uint8_t * USBD_FS_MSC_USR_BOSDescriptor(USBD_SpeedTypeDef speed, uint16_t *length);
...
__ALIGN_BEGIN uint8_t USBD_FS_MSC_DeviceDesc[USB_LEN_DEV_DESC] __ALIGN_END =
{
  ...
  LOBYTE(USBD_PID_MSC_FS),        /*idProduct*/
  HIBYTE(USBD_PID_MSC_FS),        /*idProduct*/
...
/** BOS descriptor. */
  ...
  #pragma data_alignment=4
#endif /* defined ( __ICCARM__ ) */
__ALIGN_BEGIN uint8_t USBD_FS_MSC_BOSDesc[USB_SIZ_BOS_DESC] __ALIGN_END =
{
  0x5,
  USB_DESC_TYPE_BOS,
  0xC,
...
uint8_t * USBD_FS_MSC_ProductStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{
  if(speed == 0)
  {
    USBD_GetString((uint8_t *)USBD_PRODUCT_STRING_MSC_FS, USBD_MSC_StrDesc, length);
  }
  else
  {
    USBD_GetString((uint8_t *)USBD_PRODUCT_STRING_MSC_FS, USBD_MSC_StrDesc, length);
...
{
  UNUSED(speed);
  USBD_GetString((uint8_t *)USBD_MANUFACTURER_STRING_MSC, USBD_MSC_StrDesc, length);
  return USBD_MSC_StrDesc;
}
...
uint8_t * USBD_FS_MSC_ConfigStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{
  ...
  {
    USBD_GetString((uint8_t *)USBD_CONFIGURATION_STRING_MSC_FS, USBD_MSC_StrDesc, length);
  }
  else
  {
    USBD_GetString((uint8_t *)USBD_CONFIGURATION_STRING_MSC_FS, USBD_MSC_StrDesc, length);
...
uint8_t * USBD_FS_MSC_InterfaceStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{
  if(speed == 0)
  {
    USBD_GetString((uint8_t *)USBD_INTERFACE_STRING_MSC_FS, USBD_MSC_StrDesc, length);
  }
  else
  {
    USBD_GetString((uint8_t *)USBD_INTERFACE_STRING_MSC_FS, USBD_MSC_StrDesc, length);
...
{
  UNUSED(speed);
  *length = sizeof(USBD_FS_MSC_BOSDesc);
  return (uint8_t*)USBD_FS_MSC_BOSDesc;
}
#endif /* (USBD_LPM_ENABLED == 1) */
...
static void Get_SerialNum_MSC(void)
{
  ...
  if (deviceserial0 != 0)
  {
    IntToUnicode_MSC(deviceserial0, &USBD_MSC_StringSerial[2], 8);
    IntToUnicode_MSC(deviceserial1, &USBD_MSC_StringSerial[18], 4);
  }
}
