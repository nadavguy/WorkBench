...
void updateRCState(void)
{
	...
	else if (rcLinkStatus.DownlinkPSRLQ < 50)
	{
		isNoSignal = false;
		isSignalLow = true;
	}
	...
	if (smaStatus.smaState == 0x4)
	{
		//Sound SMA Error
		previousSMAState = 0x04;
	}
	else if (smaStatus.smaState == 0x03)
	{
		//Sound SMA triggered
		previousSMAState = 0x03;
	}
	else if ((smaStatus.smaState == 0x02) && (previousSMAState != 0x02))
	{
		//Sound SMA Armed
		previousSMAState = 0x02;
	}
	else if ((smaStatus.smaState == 0x01) && (previousSMAState != 0x01))
	{
		if (isBuzzerCycleEnded)
		{
			//Sound SMA Disarmed
			previousSMAState = 0x01;
		}
	}
	else if (smaStatus.batteryVoltage < 3.8)
	{
		if (isBuzzerCycleEnded)
		{
